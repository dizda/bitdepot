...
class WithdrawCommandTest extends BaseFunctionalTestCommand
{
    ...
    public function testExecuteCreateWithdraw()
    {
        ...
        // Run the Command!
        $output = $this->runCommand('dizda:app:withdraw', ['-vvv'], true);
        ...
        $withdraw = $this->em->getRepository('DizdaAppBundle:Withdraw')->findAll();
        $withdraw = end($withdraw);

        // And assert that it was properly populated
        $this->assertEquals('0.00130000', $withdraw->getTotalInputs());
        $this->assertEquals('0.00020000', $withdraw->getTotalOutputs());
        $this->assertEquals('0.00100000', $withdraw->getFees());
        $this->assertEquals('0.00120000', $withdraw->getTotalOutputsWithFees());

        // Verify Raw Transaction
        $this->assertEquals(
            '',
        ...
        $this->assertEquals('004c69522102087a30059abeb82ceb8b0a0413c16307a0d29cec97073bbc8d4a584e60f19f2321029bfbac8f2bfca762df3ecd1500bdf291a9dad7c7491533a8b6d8925c9039432f2102a3ce2f9b90ac59d6cd5a2a01b3c1d5e9e379627ae9c9e1b2a3542f8cf80f7ae753ae', $jsonTx->inputs[0]->script);
        $this->assertEquals(130000, $jsonTx->inputs[0]->output->satoshis);
        ...
        $this->assertCount(2, $jsonTx->outputs);
        $this->assertEquals(20000, $jsonTx->outputs[0]->satoshis);
        $this->assertEquals(10000, $jsonTx->outputs[1]->satoshis);
        ...
        $this->assertEquals('3KSY3wsW6Z5a69rBSa5GdaXjsoExSUqYuK', $withdraw->getChangeAddress()->getValue());
